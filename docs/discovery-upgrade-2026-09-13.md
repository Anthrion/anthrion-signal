...
Production verification completed all eight newly queued Spanish notices in 20 HTTP attempts. The published snapshot has English title-and-description coverage for all 1,631 current public records, with no added/removed IDs or changes to original titles, descriptions, values, currencies, deadlines or first-seen dates from this translation-only release. Eleven supplementary organisation-name fields remain held by quality checks; their official names remain visible.

During verification, a scheduled translation check replaced the manual collection waiting behind deployment. GitHub's default concurrency setting retains only one pending run even with `cancel-in-progress: false`. The workflow now uses `queue: max`, retaining up to 100 waiting runs while still permitting only one active production writer. Existing recent-collection suppression prevents a delayed queue from needlessly recollecting. This does not eliminate GitHub scheduler delays or provider outages. See [GitHub's queue documentation](https://docs.github.com/en/actions/how-tos/write-workflows/choose-when-workflows-run/control-workflow-concurrency).

The follow-up production collection started at 2026-09-13T18:44:48Z and retrieved 9,100 raw notices. Scotland recovered to healthy coverage with 2,364 raw notices. The final public export contains 1,670 records with complete English title-and-description overlays: 45 additions and six removals relative to the previous 1,631-record snapshot. Seventeen additions are primarily sourced from Scotland. Retained records preserve their first-seen dates. Translation completed another 89 fields in 16 HTTP attempts; the same eleven supplementary organisation-name fields remain held by validation. Reapplying discovery to the final snapshot produces no further additions or removals.

The six removed records gained explicit expired response deadlines from newer TED notices. The Finnish ICT purchasing system previously showed a 2033 deadline; its [November correction](https://ted.europa.eu/en/notice/763263-2025/xml) explicitly states that the system was stopped on 14 November 2025 and shortens its participation and duration dates accordingly. This was verified against the official XML, not inferred from age or a downstream award. The queue change subsequently passed pipeline, frontend and full browser regression, and production deployment, in GitHub Actions run `34775893585`.
