...
Full-regression runs now use existing cached translations and leave new translation work for the next regular incremental run after verification succeeds. This removes competing work from the release budget while retaining every check. New notices can remain in their original language until the queue resumes. Translation quotas, the remote pre-call reservation, free-tier confirmation and completed-text checkpoints are unchanged.

The build job ceiling is raised from 45 minutes to two hours to give unusually long but valid work time to finish. This is a fallback, not the target runtime. The 45-minute value was a repository setting, not a GitHub requirement: [GitHub-hosted jobs allow up to six hours](https://docs.github.com/en/actions/reference/limits). Any job stopped before publication leaves the last successful site live.

[App installation tokens expire after one hour](https://docs.github.com/en/apps/creating-github-apps/authenticating-with-a-github-app/generating-an-installation-access-token-for-a-github-app), so extending the timeout alone is insufficient. Checkout no longer persists its token. The runner configures GitHub CLI as Git's credential helper, which reads the current write step's `GH_TOKEN`. A fresh token precedes translation's remote quota reservation/checkpoints; another is created after validation and tests for the final canonical-data save. Both retain Contents write permission solely for Signal. The separate successful-publication job already creates its own fresh token. If the optional translation token cannot be created, translation waits for another refresh; publication still needs its own valid credential and passing checks. No token is printed or copied into public output.
