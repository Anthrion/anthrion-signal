...
def window(feed, cursor, after, through, seen=(), reread=False):
    return {"url": feed.url(cursor), "after": (DAY0 + feed.step * after).isoformat(),
            "through": (DAY0 + feed.step * through).isoformat(), "reread": reread,
...
def test_configured_budget_finishes_a_vocabulary_replay_within_about_a_day(config):
    # Measured on 20-23 September 2026: about six collections a day, about three new archive
    # pages a day, and a replay re-reads the 14-day lookback (about 42 pages of 15-16 MB).
    spain = next(source for source in config["sources"]["sources"] if source["id"] == "spain")
    feed = GrowingFeed(60, step=timedelta(hours=8))
    results = collect_growing(feed, drained_state(feed, ["crm"]), spain["max_pages_per_run"], 8,
                              grow_every=2, replays=(0,), lookback_days=14)
    first_empty = next((index for index, result in enumerate(results) if not result.state["pending"]), None)
    assert first_empty is not None and first_empty <= 6, [len(result.state["pending"]) for result in results]
    assert len(archive_reads(feed)) <= 50


def test_vocabulary_replay_of_collected_notices_is_reported_as_current():
    feed = GrowingFeed(10)
    http = Http(transport=httpx.MockTransport(feed.handler), sleeper=lambda _: None)
    terms, settings = {"discovery_phrases": ["crm", "gestión de subvenciones"]}, {"max_pages": 80, "lookback_days": 8}
    replay = collect_spain_notices({**SOURCE, "max_pages_per_run": 3}, drained_state(feed, ["crm"]),
                                   feed.head_time() + timedelta(hours=1), http, settings, terms)
    assert replay.complete and replay.state["pending"][0]["reread"]
    assert replay.message == ("PLACSP is current; re-reading 1 older notice window from 2026-09-03 "
                              "for changed discovery vocabulary.")
    assert "watermark" not in replay.state
    feed.pages += 1
    unread = collect_spain_notices({**SOURCE, "max_pages_per_run": 2}, replay.state,
                                   feed.head_time() + timedelta(hours=1), http, settings, terms)
    assert not unread.complete and not unread.state["pending"][0]["reread"]
    assert unread.message == "PLACSP page budget reached; 2 older notice windows from 2026-09-03 will resume next run."
    caught_up = collect_spain_notices({**SOURCE, "max_pages_per_run": 3}, unread.state,
                                      feed.head_time() + timedelta(hours=1), http, settings, terms)
    assert caught_up.complete and len(caught_up.state["pending"]) == 1
    assert caught_up.message.startswith("PLACSP is current; re-reading 1 older notice window")


@pytest.mark.parametrize("newer_reread,older_reread,current", [(False, True, False), (True, True, True)])
def test_merged_window_is_a_reread_only_when_both_windows_were(newer_reread, older_reread, current):
    feed = GrowingFeed(6)
    pending = [window(feed, 5, after=2, through=6, seen=[6], reread=newer_reread),
               window(feed, 3, after=1, through=4, seen=[4], reread=older_reread)]
    http = Http(transport=httpx.MockTransport(feed.handler), sleeper=lambda _: None)
    result = collect_spain_notices({**SOURCE, "max_pages_per_run": 4}, drained_state(feed, ["crm"], pending),
                                   feed.head_time() + timedelta(hours=1), http, {"max_pages": 80}, {"discovery_phrases": ["crm"]})
    assert len(result.state["pending"]) == 1 and result.state["pending"][0]["url"] == feed.url(2)
    assert result.state["pending"][0]["after"] == (DAY0 + timedelta(days=1)).isoformat()
    assert result.complete is current


def test_head_moving_backwards_keeps_windows_and_reads_no_archive_page():
    # Observed on 22-23 September 2026: four consecutive runs stopped here, correctly.
    feed = GrowingFeed(6)
    state = {**drained_state(feed, ["crm"], [window(feed, 3, after=1, through=4, seen=[4])]),
             "head_updated": (feed.head_time() + timedelta(hours=2)).isoformat(), "head_etag": '"newer"'}
    http = Http(transport=httpx.MockTransport(feed.handler), sleeper=lambda _: None)
    result = collect_spain_notices({**SOURCE, "max_pages_per_run": 8}, state, feed.head_time() + timedelta(hours=3),
                                   http, {"max_pages": 80}, {"discovery_phrases": ["crm"]})
    assert not result.complete and result.message == "PLACSP head moved backwards; checkpoint retained"
    assert archive_reads(feed) == [] and result.state["pending"] == state["pending"]


def test_pending_window_cap_still_refuses_a_thirty_third_window():
    feed = GrowingFeed(40)
    pending = [window(feed, cursor, after=cursor - 1, through=cursor + 1) for cursor in range(1, 33)]
    state = {**drained_state(feed, ["crm"], pending), "head_updated": (DAY0 + timedelta(days=39)).isoformat(),
             "head_hash": "older head", "head_etag": '"older"'}
    http = Http(transport=httpx.MockTransport(feed.handler), sleeper=lambda _: None)
    result = collect_spain_notices({**SOURCE, "max_pages_per_run": 8}, state, feed.head_time() + timedelta(hours=1),
                                   http, {"max_pages": 80}, {"discovery_phrases": ["crm"]})
    assert not result.complete and "pending window limit reached" in result.message
    assert len(result.state["pending"]) == 32 and archive_reads(feed) == []
