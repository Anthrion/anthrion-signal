...
The reviewed-record pilot adds source-bound decisions in `config/record_reviews.json`: individual exclusions for clearly unrelated scope, and concise **Recommended approach:** / **Problems:** guidance for selected current UK opportunities. A primary reviewer checks model proposals against the complete retained notice and available lots. Unknown qualifications do not disqualify a supplier, and mixed or uncertain digital scope remains visible. Original records remain retained; changed source scope invalidates the decision. Complexity and problem-level estimates are stored with guidance for future filtering but are not displayed or used to rank the feed. No model runs during browsing or automatically approves exclusions. See [the review process and first audit](docs/reviewed-records.md).
