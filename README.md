...
Rejected notices use one current record per notice ID plus reversible field changes for earlier versions. New observations update the current version only when their publisher timestamp is at least as recent; older observations remain in history. Source text, metadata and all distinct prior versions can be reconstructed exactly. A verified migration replaces the legacy daily files with 256 bounded gzip buckets. Separately, daily maintenance can remove old, explicitly physical/non-technical rejected notices only when every version passes conservative retention checks and none is used by a current record, award, research page, review, translation or related procurement. All canonical and archived records stay retained. See [retention policy and recovery](docs/retained-notice-history.md).
...
Gemini-based scoring, its SDK dependency and score-based product features have been retired. Historical canonical analysis and offline validation helpers remain for migration/history, but public serialization strips that legacy analysis. Gemini is used separately for private, cached English translation only. Old score-filter URLs migrate to source-only views. Legacy `--no-ai` remains accepted; nonzero `--max-ai` is rejected.

Reviewed records use source-bound decisions in `config/record_reviews.json`: individual exclusions for clearly unrelated scope, and concise **Recommended approach:** / **Problems:** guidance for selected current opportunities in the UK, United States, Canada, Germany, Austria and Switzerland. Recommendations describe concrete products, configuration, custom components and integration responsibilities. The existing English/Original choice also selects reviewed recommendations and problem statements, preserving the setting across pages without another translation request. A separate reviewer checks proposals against complete retained notices and available lots; the release review validates evidence, source hashes and language alignment. Unknown qualifications do not disqualify a supplier, and mixed or uncertain digital scope remains visible. Original records remain retained; changed source scope invalidates the decision. Complexity and problem-level estimates stay in record details for future filtering and do not affect ranking. Guidance does not enlarge search indexes. No model runs during browsing or automatically approves exclusions. See [the review process and audit](docs/reviewed-records.md) and [the technical authoring guide](docs/recommendation-review-guide.md).
...
The translation worker covers current opportunities and the same relevant historical awards selected for publication. Current notice titles and descriptions run first, followed by award text, then supplementary buyer names. Both groups interleave countries and reuse completed source-hash entries; awards do not enter the live feed. Each run reports complete notice pairs separately for current and awarded records. Historical backfill shares the existing free-tier quota and progresses as capacity becomes available. The growing field cache and English sidecar use lossless gzip storage above 32 MiB. Public market indexes, full records, buyer timelines and the complete-feed fallback also use lossless gzip; the bootstrap manifest remains JSON. Browser, validation and deployment readers support both formats.
...
Independently checked complete translations can also be committed in `config/reviewed_translations.json`. These reuse the existing source-text hash, literal/protected-name checks and English validation. They supplement the provider cache without changing the originals or making API requests.
...
PRs keep the required `test` check without rebuilding the full public dataset: frontend unit tests, fixture-backed production compilation and critical desktop/mobile journeys. Pipeline, contract, dependency, automation and unknown changes also run the Python suite and relevance benchmark. The full browser regression suite and real-data integration remain required before publication; failed releases leave the existing live site in place. Fixture preparation runs only on PRs, cannot overwrite an existing export, and creates no release receipt.
