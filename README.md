...
The translation worker covers current opportunities and the same relevant historical awards selected for publication. Current notice titles and descriptions run first, followed by award text, then supplementary buyer names. Both groups interleave countries and reuse completed source-hash entries; awards do not enter the live feed. Each run reports complete notice pairs separately for current and awarded records. Historical backfill shares the existing free-tier quota and progresses as capacity becomes available. The growing field cache and English sidecar use the existing lossless gzip storage above 32 MiB; public browser files remain ordinary JSON.
